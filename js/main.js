...
getId('search-button').addEventListener('click', () => {
    ...
    getId('meal-container').innerHTML = ''; //clear past meals list

    const regexp = /[a-zA-z]/; //set regular expression
    //check input is name or not
    if (searchName.match(regexp)) {
        getMealsList(searchName);
    } else {
        getId('message').innerText = 'Please Search Foods By Name.';
    }
    getId('search-input').value = ''; //clear all value after search
});
...
const getMealsList = async searchName => {
    ...
    if (data.meals) {
        ...
        getId('message').innerText = `The "${searchName}" Oops!Food Are Not Found! Please Try Another One`;
    }
};
